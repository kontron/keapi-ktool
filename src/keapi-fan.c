...
const char fan_usage[] = "\
    * GetFanSensorCount\n\
    * GetFanSensorValue <fan_sensor_number>\n\
    * GetFanSensorValueList\n\
    * GetFanSensorInfo <fan_sensor_number>\n\
...
int fan_call(const char *FunctName, int argc, const char *argv[])
{
	uint32_t retvalue;

	if (strncasecmp(FunctName, "GetFanSensorCount", KEAPI_MAX_STR) == 0) {
		static int32_t fan_count = 0;
		CHECK_PARAMS(0);

		retvalue = KEApiGetFanSensorCount(&fan_count);
		if (retvalue == KEAPI_RET_SUCCESS)
			printf("Fan count: %d\n", fan_count);

	} else if (strncasecmp(FunctName, "GetFanSensorValue", KEAPI_MAX_STR) == 0) {
		KEAPI_SENSOR_VALUE sensor_value = { 0 };
		int32_t sensor_number = 0;
		CHECK_PARAMS(1);

		sensor_number = atoi(argv[0]);
		retvalue = KEApiGetFanSensorValue(sensor_number, &sensor_value);
		if (retvalue == KEAPI_RET_SUCCESS) {
	...
	} else if (strncasecmp(FunctName, "GetFanSensorValueList", KEAPI_MAX_STR) == 0) {
		PKEAPI_SENSOR_VALUE psensor_values = NULL;
		int32_t sensor_count = 0;
		CHECK_PARAMS(0);

		if ((retvalue = KEApiGetFanSensorCount(&sensor_count)) == KEAPI_RET_SUCCESS) {
			...
			if ((retvalue = KEApiGetFanSensorValueList(psensor_values, sensor_count)) ==
			    KEAPI_RET_SUCCESS) {
				int32_t i;
			...
			keapi_perror("KEApiGetFanSensorCount failed", retvalue);

	} else if (strncasecmp(FunctName, "GetFanSensorInfo", KEAPI_MAX_STR) == 0) {
		KEAPI_SENSOR_INFO sensor_info = { { 0 } };
		int32_t sensor_number = 0;
		CHECK_PARAMS(1);

		sensor_number = atoi(argv[0]);
		retvalue = KEApiGetFanSensorInfo(sensor_number, &sensor_info);
		if (retvalue == KEAPI_RET_SUCCESS) {
			printf("Sensor number: %d\n", sensor_number);
